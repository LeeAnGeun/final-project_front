...
WHERE MEMNUM=7
	...
	SELECT X.RNUM, X.CLASSNUM AS CLASSNUM, INSTRUCTOR, MASTERNUM, TITLE, IMAGE1, ENDDATE, SECONDARYCATEGORY, PRIMARYCATEGORY, PRICE, EMAIL, PROFILEPIC, NVL(AVGPOINT, 0)
	FROM 
	     (SELECT ROW_NUMBER()OVER(ORDER BY O.STARTDATE) AS RNUM, O.CLASSNUM AS CLASSNUM, INSTRUCTOR, MASTERNUM, TITLE, IMAGE1, EMAIL, PROFILEPIC, STARTDATE, ENDDATE, SECONDARYCATEGORY, PRIMARYCATEGORY, PRICE
		  FROM LIKES L, ACLAPMEMBER A, ONEDAYCLASS O
		  WHERE L.MEMNUM = A.MEMNUM AND L.CLASSNUM = O.CLASSNUM AND A.MEMNUM=7
		  ORDER BY O.STARTDATE) X,  
		 (SELECT CLASSNUM, AVG(STARPOINT) AS AVGPOINT
		  FROM REVIEW
	      WHERE MEMNUM != (SELECT MEMNUM FROM ACLAPMEMBER WHERE MEMNUM = 1)
          GROUP BY CLASSNUM) Y
	WHERE X.CLASSNUM = Y.CLASSNUM AND X.RNUM BETWEEN 1 AND 5
	ORDER BY X.RNUM	
	
	SELECT DISTINCT ROW_NUMBER()OVER(ORDER BY O.STARTDATE) AS RNUM, O.CLASSNUM AS CLASSNUM, INSTRUCTOR, MASTERNUM, TITLE, IMAGE1, 
	EMAIL, PROFILEPIC, STARTDATE, ENDDATE, SECONDARYCATEGORY, PRIMARYCATEGORY, PRICE, AVGPOINT
	FROM LIKES L, ACLAPMEMBER A, ONEDAYCLASS O, (SELECT CLASSNUM, AVG(STARPOINT) AS AVGPOINT
		  FROM REVIEW
		  WHERE MEMNUM != 1
		  GROUP BY CLASSNUM) R
    WHERE L.MEMNUM = A.MEMNUM AND L.CLASSNUM = O.CLASSNUM AND L.CLASSNUM = R.CLASSNUM AND A.MEMNUM=7
    GROUP BY CLASSNUM
...
---별점 평균---
SELECT AVG(STARPOINT) AS AVGPOINT
FROM REVIEW R, ONEDAYCLASS O
WHERE R.CLASSNUM = O.CLASSNUM AND O.CLASSNUM = 7
